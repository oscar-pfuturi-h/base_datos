...
<?php 
		$qry = "SELECT c.id_cliente, c.nombre, c.apellidos, c.correo, 
					COUNT(r.fecha_reservada) 'nro_reservas'
				FROM registro_cliente rc
				LEFT JOIN clientes c ON c.id_cliente=rc.id_cliente
				LEFT JOIN reservaciones r ON r.id_reserva=rc.id_reserva
...
?>
			<tr class="row">
				...
				<td class="data cn-reservas info user"><?php echo $filas['nro_reservas']; ?>
				</td>
	<?php 
			$qry = "SELECT fecha_reservada, hora FROM reservaciones
					WHERE id_cliente = ".$filas['id_cliente']." AND fecha_reservada >= CURRENT_DATE
					ORDER BY fecha_reservada DESC";

			$res1 = $db_connection->query($qry);
			if ($res1->num_rows == 0){
	?>
			...
			?>
				<td class="ver"><button type="button" name="f-btn" id="factura-btn" onclick="window.location.href='reservaciones.php?cid=<?php echo $filas['id_cliente']; ?>'">Reservaciones</button></td>
				<td class="ver"><button type="button" name="f-btn" id="factura-btn" onclick="window.location.href='facturas.php?cid=<?php echo $filas['id_cliente']; ?>'">Facturas</button></td>
			</tr>
		<?php 
		}
		?>
...
?>
			...
			?>
				...
				<td id="remove" class="buttons"><figure>
					<img src="img/tacho.jpg" onclick="window.location.href='eliminar_prod.php'" title="Eliminar">
				</figure></td>
			</tr>
			<?php 
			}
...
?>
			<tr class="emp-row">
				...
				<td id="edition" class="buttons"><figure>
					<img src="img/editar.jpg" onclick="window.location.href='editar_empleado.php'" title="Editar">
				</figure></td>
				<td id="remove" class="buttons"><figure>
					<img src="img/tacho.jpg" onclick="window.location.href='eliminar_empleado.php'" title="Eliminar">
				</figure></td>
			</tr>
		<?php 
		}
